<script setup>
  ...
  const updateTask = async (taskId, payload) => {
    try {
      ...
      errorMsg.value =
        "There's been an error while updating your task:" + error;
    }
  };
...
<template>
  ...
  <main
    class="p-8 grid grid-cols-1 gap-10 auto-rows-max lg:auto-rows-fr lg:grid-cols-3 min-height-empty"
  >
    <div>
    ...
    <section class="lg:col-span-2">
      ...
      <h5
        v-if="
          ...
          (filterNumber !== '0' || filterDateValue !== 'all')
        "
        class="text-lg mt-10"
      >
      ...
      <h5
        ...
        class="text-lg mt-10"
      >
        Come on! Start planning tasks here ;)
      </h5>
      <section
        v-if="tasksCompleted.length > 0 && showCompletedTasks"
        class="grid gap-10 mt-10"
